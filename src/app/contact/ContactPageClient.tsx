...
import type { resolveCampuses } from "@/data/site";

type Campus = ReturnType<typeof resolveCampuses>[number];
...
export function ContactPageClient({
  ...
  const visible = campuses.filter((campus) => {
    if (isKids) return campus.id === "kids";
    if (isInstitute) return campus.id === "institute";
    return true;
  });

  const heroCopy = isKids
    ? {
        title: "Contact OP Kids",
        subtitle:
          "Reach OP Kids Pre School at Mahavir Enclave Part 2 — phone, email, WhatsApp, and campus visit.",
      }
    : isInstitute
      ? {
          title: "Contact the Institute",
          subtitle:
            "Reach OP Institute of Studies at Mahavir Enclave Part 2 — phone, email, WhatsApp, and campus visit.",
        }
      : {
          title: "Contact Us",
          subtitle:
            "Two Google listings at Mahavir Enclave Part 2 — OP Institute of Studies and OP Kids Pre School, each with its own phone, email, and social links.",
        };

  const sideEmail = isKids ? extras.kidsEmail : extras.email;
  const sideWhatsapp = isKids ? extras.kidsWhatsapp : extras.whatsapp;
  const sideHours = isKids
    ? `${extras.preschoolHours} · ${extras.sundayHours}`
    : isInstitute
      ? `${extras.weekdayHours} · ${extras.sundayHours}`
      : `${extras.weekdayHours} · ${extras.preschoolHours} · ${extras.sundayHours}`;
    ...
    <>
      ...
      <section className="section-padding">
        <div className="container-custom">
          <div
            className={cn(
              "grid gap-4 sm:gap-6 lg:gap-8 mb-8 sm:mb-12",
              visible.length > 1 ? "md:grid-cols-2" : "max-w-2xl mx-auto"
            )}
          >
              ...
              <div className="space-y-4">
                {[
                  ...
                  },
                  {
                    ...
                    label: "WhatsApp",
                    value: isKids
                      ? "Chat with OP Kids team"
                      : isInstitute
                        ? "Chat with Institute admissions"
                        : "Chat with admissions team",
                    href: `https://wa.me/${sideWhatsapp}`,
                  },
                  {
                    icon: Clock,
                  ...
                  >
                    <div
                      className={cn(
                        ...
                        isKids
                          ? "bg-kids-100 dark:bg-kids-900/30"
