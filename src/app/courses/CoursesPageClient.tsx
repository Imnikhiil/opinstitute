          ...
          <div className="flex flex-wrap justify-center gap-1.5 sm:gap-2 mb-8 sm:mb-12">
            {courseCategories.map((cat) => (
              <button
                ...
                className={cn(
                  "px-3 py-2 sm:px-5 sm:py-2.5 rounded-full text-xs sm:text-sm font-medium transition-all",
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            {filtered.map((course, index) => (
              <ScrollReveal key={course.id} delay={index * 0.08}>
                <div className="glass-card p-4 sm:p-6 md:p-8 h-full flex flex-col hover:shadow-card-hover transition-all hover:-translate-y-1 relative">
                  {course.popular && (
                  ...
                  <h2 className="font-display font-bold text-xl sm:text-2xl mb-2 sm:mb-3">
                    {course.name}
                  </h2>
