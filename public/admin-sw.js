/* Admin-only service worker — scope /admin (see next.config headers) */
const CACHE = "op-admin-v2";
...
self.addEventListener("fetch", (event) => {
  ...
  // Network-first for admin pages (always fresh CMS data when online)
  event.respondWith(
    fetch(request)
      .then((response) => {
        const copy = response.clone();
        if (response.ok && url.pathname.startsWith("/admin-pwa")) {
